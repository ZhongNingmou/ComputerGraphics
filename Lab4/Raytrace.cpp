...
// Compute for shadow if hasShadow is true.
    ...
    for (const auto& light : scene.ptLights) {
        Vector3d L = (light.position - nearestHitRec.p).makeUnitVector();
        int flag = 1;
        if (hasShadow) {
            Ray tempRay(nearestHitRec.p, L);
            double tMax = (light.position - nearestHitRec.p).length();
            for (const auto& surface : scene.surfaces) {
                if (surface->shadowHit(tempRay, DEFAULT_TMIN, tMax)) {
                    flag = 0;
                    break;
                }
            }
        }
        result += flag * computePhongLighting(L, N, V, nearestHitRec.material, light);
    }
...
// Add to result the reflection of the scene.
    ...
    for (int i = 0; i < reflectLevels; i++) {
        Vector3d R = 2 * dot(-uRay.direction(), N) * N + uRay.direction();
        result += nearestHitRec.material.k_rg * Raytrace().TraceRay(
                Ray(nearestHitRec.p, R), scene, reflectLevels - 1, hasShadow);
    }
