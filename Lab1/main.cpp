...
void DrawDisc( const discType *d )
{
    ...
    if ( firstTime )
    {
        ...
        {
            unitDiscVertex[i][0] = 1 * cos(i * 2 * PI/NUM_OF_SIDES);
            unitDiscVertex[i][1] = 1 * sin(i * 2 * PI/NUM_OF_SIDES);
        }
    ...
    glBegin(GL_TRIANGLE_FAN);
        ...
        for (int i = 0; i < NUM_OF_SIDES + 1; i++)
        {
            glVertex2f(d->radius * unitDiscVertex[i][0] + d->pos[0], d->radius * unitDiscVertex[i][1] + d->pos[1]);
        }
    glEnd();
    glFlush();
}
...
void MyMouse( int btn, int state, int x, int y )
{
    if ( btn == GLUT_LEFT_BUTTON && state == GLUT_DOWN )
    {
        ...
        else
        {
            ...
            int sign = rand() % (1 - (-1) + 1);
            sign = sign > 0 ? 1 : -1;
            disc[ numDiscs ].speed[0] = sign * rand() % (int)(MAX_X_SPEED - MIN_X_SPEED + 1);
            disc[ numDiscs ].speed[1] = sign * rand() % (int)(MAX_Y_SPEED - MIN_Y_SPEED + 1);
            disc[ numDiscs ].color[0] = 0 + rand() % (255 - 0 + 1);
            disc[ numDiscs ].color[1] = 0 + rand() % (255 - 0 + 1);
            disc[ numDiscs ].color[2] = 0 + rand() % (255 - 0 + 1);
            numDiscs++;
            glutPostRedisplay();
        }
    }
...
int main( int argc, char** argv )
{
    ...
    glutTimerFunc(0, MyTimer, 0);  //*** MODIFY THIS ***
