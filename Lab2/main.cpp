...
void DrawOneCar( float bodyColor[3] )
{
    ...
    glScaled(CAR_LENGTH, CAR_WIDTH, CAR_HEIGHT / 2);
    glutSolidCube(1);
    glPopMatrix();

    glPushMatrix();
    glTranslated(0, 0, CAR_HEIGHT / 2);
    glScaled(CAR_LENGTH / 2, CAR_WIDTH, CAR_HEIGHT / 2);
    glutSolidCube(1);
    glPopMatrix();
    //****************************
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (i < 2) {
            if (i % 2 == 0) {
                glTranslated( - CAR_LENGTH / 4, - CAR_WIDTH / 2, CAR_HEIGHT / 5);
            }
            else {
                glTranslated( - CAR_LENGTH / 4, CAR_WIDTH / 2, CAR_HEIGHT / 5);
            }
        ...
        glRotated(90, 1, 0, 0);
        glutSolidTorus(CAR_HEIGHT / 10, CAR_HEIGHT / 5, 20, 20);
        glPopMatrix();
    //****************************
    }
}
...
void MyDisplay( void )
{
    ...
    //***********************************************************************
    gluLookAt( eyeDistance * sin(eyeLongitude*PI/180) * cos(eyeLatitude*PI/180),
